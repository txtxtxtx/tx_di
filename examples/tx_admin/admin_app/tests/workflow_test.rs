...
async fn full_crud_workflow() {
    // 1. 创建部门层级
    let (dept_app, _, _) = common::create_dept_app();
    ...
    // 2. 创建角色
    let (role_app, _, _) = common::create_role_app();
    ...
    // 3. 创建用户（含角色和部门）
    let (user_app, _, _) = common::create_user_app();
    ...
    // 4. 创建菜单层级并分配给角色
    let (menu_app, _, _) = common::create_menu_app();
    ...
    // 5. 创建系统配置
    let (config_app, _, _) = common::create_config_app();
    ...
    // 7. 文件上传
    let (file_app, _, _) = common::create_file_app();
    ...
    // 8. 日志记录
    let (log_app, _, _) = common::create_operate_log_app();
