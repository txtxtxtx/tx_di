...
async fn full_crud_workflow() {
    ...
    let root_dept = dept_app.create_dept(CreateDeptRequest {
        name: "总公司".into(), parent_id: 0, sort: 1,
        leader_user_id: None, phone: None, email: None,
    }, Some("admin".into())).await.unwrap();
    let tech_dept = dept_app.create_dept(CreateDeptRequest {
    ...
    let admin_role = role_app.create_role(CreateRoleRequest {
        name: "管理员".into(), code: "admin".into(), sort: 1,
        remark: None, menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();
    let user_role = role_app.create_role(CreateRoleRequest {
    ...
    let sys_menu = menu_app.create_menu(CreateMenuRequest {
    ...
    let user_menu = menu_app.create_menu(CreateMenuRequest {
    ...
    config_app.create_config(CreateConfigRequest {
    ...
    dict_type_app.create_dict_type(CreateDictTypeRequest {
    ...
    for (i, label) in ["男", "女"].iter().enumerate() {
        dict_data_app.create_dict_data(CreateDictDataRequest {
    ...
    // 9. 验证配置管理分页
    let cfg_page = config_app.get_config_page(ListConfigsRequest {
