...
use admin_proto::{CreateOperateLogRequest, CreateUserRequest};
use std::io::Cursor;
...
async fn full_crud_workflow() {
    ...
    let user = user_app
        .create_user(
            CreateUserRequest {
                ...
                nickname: "张三".into(),
                email: Some("zs@example.com".into()),
                mobile: Some("13800000001".into()),
    ...
    let user_menu = menu_app
        .create_menu(
            CreateMenuRequest {
                ...
                component_name: Some("UserMgmt".into()),
            },
            Some("admin".into()),
        )
        .await
        .unwrap();
    role_app
        .assign_menus(admin_role.id, vec![sys_menu.id, user_menu.id])
        .await
        .unwrap();
    ...
    let items = dict_data_app
        .get_by_dict_type("sys_user_sex")
        .await
        .unwrap();
