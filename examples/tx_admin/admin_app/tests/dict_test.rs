...
mod common;
use admin_proto::{
    CreateDictDataRequest, CreateDictTypeRequest, ListDictDataRequest, ListDictTypesRequest,
    UpdateDictDataRequest, UpdateDictTypeRequest,
};
...
async fn create_dict_type_success() {
    ...
    let dt = app
        .create_dict_type(req, Some("admin".into()))
        .await
        .unwrap();
...
async fn create_duplicate_dict_type_should_fail() {
    ...
    app.create_dict_type(req("dup_type"), Some("admin".into()))
        .await
        .unwrap();
    assert!(
        app.create_dict_type(req("dup_type"), Some("admin".into()))
...
async fn delete_dict_type() {
    ...
    app.delete_dict_type(dt.id, Some("admin".into()))
        .await
        .unwrap();
}
...
async fn paginate_dict_types() {
    let (app, _, _) = common::create_dict_type_app().await;
    for i in 0..4 {
        app.create_dict_type(
            CreateDictTypeRequest {
                name: format!("类型{}", i),
                dict_type: format!("type_{}", i),
                remark: None,
            },
            Some("admin".into()),
        )
    ...
    let page = app
        .get_dict_type_page(ListDictTypesRequest {
            name: None,
            dict_type: None,
            status: None,
            page: 1,
...
async fn create_dict_data_success() {
    ...
    let dd = app
        .create_dict_data(req, Some("admin".into()))
        .await
        .unwrap();
...
async fn delete_dict_data() {
    ...
    app.delete_dict_data(dd.id, Some("admin".into()))
        .await
        .unwrap();
}
...
async fn paginate_dict_data() {
    ...
    let page = app
        .get_dict_data_page(ListDictDataRequest {
            dict_type: Some("test".into()),
            label: None,
            status: None,
            page: 1,
            page_size: 2,
