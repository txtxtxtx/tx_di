...
async fn create_duplicate_dict_type_should_fail() {
    ...
    app.create_dict_type(cmd("dup_type"), Some("admin".into())).await.unwrap();
    assert!(app.create_dict_type(cmd("dup_type"), Some("admin".into())).await.is_err());
}
...
async fn update_dict_type() {
    ...
    let updated = app.update_dict_type(UpdateDictTypeCommand {
        id: dt.id,
        name: "新名称".into(),
...
async fn create_dict_data_success() {
    ...
    let dd = app.create_dict_data(cmd, Some("admin".into())).await.unwrap();
...
async fn update_dict_data() {
    ...
    let updated = app.update_dict_data(UpdateDictDataCommand {
        id: dd.id,
        sort: 10,
        label: "新标签".into(),
        value: "new".into(),
