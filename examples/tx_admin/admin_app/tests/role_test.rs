...
async fn create_role_success() {
    ...
    let role = app.create_role(CreateRoleRequest {
        ...
        remark: Some("系统管理员".into()), menu_ids: vec![],
...
async fn create_duplicate_code_should_fail() {
    ...
    let cmd = |code: &str, name: &str| CreateRoleRequest {
        name: name.into(), code: code.into(), sort: 1, remark: None, menu_ids: vec![],
    };
...
async fn update_role_success() {
    let (app, _, _) = common::create_role_app().await;
    let role = app.create_role(CreateRoleRequest {
        name: "编辑角色".into(), code: "editor".into(), sort: 1, remark: None, menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();

    let updated = app.update_role(UpdateRoleRequest {
        role_id: role.id,
        name: "高级编辑".into(),
...
async fn delete_role_success() {
    let (app, _, _) = common::create_role_app().await;
    let role = app.create_role(CreateRoleRequest {
        name: "待删除".into(), code: "todel".into(), sort: 99, remark: None, menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();
...
async fn get_role_detail() {
    ...
    let role = app.create_role(CreateRoleRequest {
        ...
        remark: Some("备注信息".into()), menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();
...
async fn paginate_roles() {
    let (app, _, _) = common::create_role_app().await;
    for i in 0..5 {
        app.create_role(CreateRoleRequest {
            name: format!("角色{}", i), code: format!("r{}", i), sort: i,
            remark: None, menu_ids: vec![],
        }, Some("admin".into())).await.unwrap();
    }
    let page = app.get_role_page(ListRolesRequest {
...
async fn query_role_by_name_fuzzy() {
    let (app, _, _) = common::create_role_app().await;
    app.create_role(CreateRoleRequest {
        name: "系统管理员".into(), code: "sys_admin".into(), sort: 1, remark: None, menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();
    app.create_role(CreateRoleRequest {
        name: "普通用户".into(), code: "user".into(), sort: 2, remark: None, menu_ids: vec![],
    }, Some("admin".into())).await.unwrap();
