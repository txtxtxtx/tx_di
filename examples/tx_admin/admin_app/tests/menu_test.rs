...
async fn create_menu_hierarchy() {
    ...
    let child = app.create_menu(CreateMenuCommand {
...
async fn update_menu() {
    ...
    let updated = app.update_menu(UpdateMenuCommand {
        menu_id: menu.id,
        name: "新名称".into(),
...
async fn get_menu_tree() {
    ...
    let tree = app.get_menu_tree(MenuQueryRequest { name: None, status: None, types: None }).await.unwrap();
...
async fn get_menu_list_flat() {
    ...
    let list = app.get_menu_list(MenuQueryRequest { name: None, status: None, types: None }).await.unwrap();
