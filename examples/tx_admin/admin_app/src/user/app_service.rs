...
impl UserAppService {
    ...
    pub async fn create_user(
        ...
        {
            return Err(RepositoryError::DuplicateEmail.into());
        }
        ...
        {
            return Err(RepositoryError::DuplicateMobile.into());
        }
    ...
    pub async fn assign_roles(&self, user_id: u64, role_ids: Vec<u64>) -> AppResult<()> {
        ...
        if roles.len() != role_ids.len() {
            return Err(RepositoryError::NotFoundRole.into());
        }
        for r in &roles {
            if r.status != 0 {
                return Err(RepositoryError::ValidationUserStatus.into());
    ...
    pub async fn assign_departments(&self, user_id: u64, dept_ids: Vec<u64>) -> AppResult<()> {
        ...
        if depts.len() != dept_ids.len() {
            return Err(RepositoryError::NotFoundDept.into());
        }
        for d in &depts {
            if d.status != 0 {
                return Err(RepositoryError::ValidationDeptDisabled.into());
            }
        }
