...
//!
//! `tx_di_nacos::app_loop!` 在 App 启动后统一收集端点并注册到 Nacos；
//! 本模块通过 `register_endpoints` 在 `AdminPlugin`（`app_async_init`）中
//! 提前提供端点，保证收集顺序正确。

use std::collections::HashMap;

use tx_di_nacos::{EndpointProvider, Protocol, ServiceEndpoint};
