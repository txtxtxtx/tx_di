...
use tx_di_axum::aide::axum::routing::{delete, get, post, put};
...
async fn create_dept(
    ...
    match dept.create_dept(cmd, None).await {
        Ok(r) => R(ApiR::success(map_dept(r))),
...
async fn get_dept(
    ...
    match dept.get_dept(dept_id).await {
        Ok(r) => R(ApiR::success(map_dept(r))),
        Err(e) => R(ApiRes::from(e).into_typed()),
    }
}
...
async fn update_dept(
    ...
    match dept.update_dept(cmd, None).await {
        Ok(r) => R(ApiR::success(map_dept(r))),
        Err(e) => R(ApiRes::from(e).into_typed()),
    }
}
...
async fn delete_dept(
    ...
    match dept.delete_dept(dept_id, None).await {
...
async fn list_depts(
    ...
    match dept.get_dept_tree(q).await {
        Ok(tree) => R(ApiR::success(tree)),
        Err(e) => R(ApiRes::from(e).into_typed()),
    }
}
