...
use crate::identity::menu::model::event::MenuEvent;

/// Menu aggregate root
#[derive(Debug, Clone, Serialize, Deserialize, AggregateRoot)]
#[aggregate_root(event = crate::identity::menu::model::event::MenuEvent)]
/// 菜单结构体，用于存储菜单相关的信息
...
pub struct Menu {
    ...
    /// 领域事件列表，使用MenuEvent类型的向量存储，外部不可见
...
impl Menu {
    ...
    pub fn create(
        ...
        menu.add_event(MenuEvent::MenuCreated { menu_id: id });
        menu
    }
    ...
    pub fn update_info(
        ...
        self.add_event(MenuEvent::MenuUpdated { menu_id: self.id });
    }
    ...
    pub fn soft_delete(&mut self, updater: Option<String>) {
        ...
        self.audit.updater = updater;
        self.audit.update_time = Timestamp::now();
        self.add_event(MenuEvent::MenuDeleted { menu_id: self.id });
    }
