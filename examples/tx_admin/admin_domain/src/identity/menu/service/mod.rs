...
use crate::identity::menu::model::value_object::{MenuQuery, MenuTreeNode};
use crate::identity::menu::repository::MenuRepository;
use crate::shared::model::value_object::DeletedStatus;
use crate::identity::menu::repository::MenuRepositoryError;
...
impl MenuService {
    ...
    pub async fn update_menu(
        ...
        let mut menu = self
            ...
            .find_by_id(menu_id)
            .await?
            .ok_or(MenuRepositoryError::NotFoundMenu)?;

        // Cannot set self as parent
        if parent_id == menu_id {
            Err(MenuRepositoryError::ValidationMenuSelfParent)?;
        }
    ...
    pub async fn delete_menu(&self, menu_id: u64, updater: Option<String>) -> AppResult<()> {
        if self.menu_repo.has_children(menu_id).await? {
            Err(MenuRepositoryError::ValidationMenuHasChildren)?;
        }
