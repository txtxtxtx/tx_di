...
mod user_tests {
    ...
    use crate::identity::user::model::value_object::{Sex, UserStatus};
    ...
    fn test_create_user_raises_user_created_event() {
        ...
        match &user.events()[0] {
            UserEvent::UserCreated { user_id, username } => {
                assert_eq!(*user_id, 1);
    ...
    fn test_set_basic_info_raises_user_updated_event() {
        ...
        assert!(matches!(
            user.events().last(),
            Some(UserEvent::UserUpdated { user_id: 1 })
        ));
    }
    ...
    fn test_change_status_raises_user_status_changed_event() {
        ...
        match user.events().last().unwrap() {
            UserEvent::UserStatusChanged { user_id, status } => {
                assert_eq!(*user_id, 1);
    ...
    fn test_change_password_raises_event() {
        ...
        assert!(matches!(
            user.events().last(),
            Some(UserEvent::UserPasswordChanged { user_id: 1 })
        ));
    }
    ...
    fn test_record_login_raises_user_logged_in_event() {
        ...
        assert_eq!(user.events().len(), before + 1);
        match user.events().last().unwrap() {
            UserEvent::UserLoggedIn { user_id, ip } => {
                assert_eq!(*user_id, 1);
    ...
    fn test_soft_delete_raises_user_deleted_event() {
        ...
        assert!(matches!(
            user.events().last(),
            Some(UserEvent::UserDeleted { user_id: 1 })
        ));
    }
