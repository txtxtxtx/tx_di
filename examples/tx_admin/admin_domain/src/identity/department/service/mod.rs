...
use crate::identity::department::model::value_object::{DeptQuery, DeptTreeNode};
use crate::identity::department::repository::DepartmentRepository;
use crate::shared::model::value_object::DeletedStatus;
use crate::identity::department::repository::DepartmentRepositoryError;
use std::sync::Arc;
use tx_common::id;
...
impl DepartmentService {
    ...
    pub async fn update_dept(
        ...
        let mut dept = self
            ...
            .find_by_id(dept_id)
            .await?
            .ok_or(DepartmentRepositoryError::NotFoundDept)?;

        if parent_id == dept_id {
            Err(DepartmentRepositoryError::ValidationDeptSelfParent)?;
        }
    ...
    pub async fn delete_dept(&self, dept_id: u64, updater: Option<String>) -> AppResult<()> {
        if self.dept_repo.has_children(dept_id).await? {
            Err(DepartmentRepositoryError::ValidationDeptHasChildren)?;
        }
        if self.dept_repo.has_users(dept_id).await? {
            Err(DepartmentRepositoryError::ValidationDeptHasUsers)?;
        }
