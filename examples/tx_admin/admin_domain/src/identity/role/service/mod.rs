...
use crate::identity::role::repository::RoleRepository;
use crate::identity::role::repository::RoleRepositoryError;
use std::sync::Arc;
use tx_common::id;
...
impl RoleService {
    ...
    pub async fn create_role(
        ...
        if self.role_repo.exists_by_code(&code).await? {
            Err(RoleRepositoryError::DuplicateRoleCode)?;
        }
    ...
    pub async fn update_role(
        ...
        let mut role = self
            ...
            .find_by_id(role_id)
            .await?
            .ok_or(RoleRepositoryError::NotFoundRole)?;
    ...
    pub async fn assign_menus(&self, role_id: u64, menu_ids: Vec<u64>) -> AppResult<Role> {
        ...
        // 角色必须为启用状态才能分配菜单
        if role.status != 0 {
            Err(RoleRepositoryError::ValidationRoleDisabled)?;
        }
    ...
    pub async fn get_role_users(
        &self,
        role_id: u64,
    ) -> AppResult<Vec<crate::identity::user::model::aggregate::User>> {
        // Verify role exists
        let _role = self
            .role_repo
            .find_by_id(role_id)
