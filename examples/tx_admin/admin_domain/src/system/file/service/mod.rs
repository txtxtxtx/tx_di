...
use crate::system::file::model::value_object::{FileDownloadInfo, FileQuery, FileUploadCommand};
use crate::system::file::repository::{FileConfigRepository, FileRepository};
use crate::system::file::repository::FileRepositoryError;
use tx_common::id;
...
impl FileService {
    ...
    pub async fn delete_file(&self, file_id: u64, updater: Option<String>) -> AppResult<()> {
        let mut file = self
            ...
            .find_by_id(file_id)
            .await?
            .ok_or(FileRepositoryError::NotFoundFile)?;
