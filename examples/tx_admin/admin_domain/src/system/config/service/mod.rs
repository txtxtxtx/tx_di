...
use crate::system::config::repository::ConfigRepository;
use crate::system::config::repository::ConfigRepositoryError;
use tx_common::id;
...
impl ConfigService {
    ...
    pub async fn create_config(
        ...
        if self.config_repo.exists_by_key(&config_key).await? {
            Err(ConfigRepositoryError::DuplicateConfigKey)?;
        }
    ...
    pub async fn update_config(
        ...
        let mut config = self
            ...
            .await?
            .ok_or(ConfigRepositoryError::NotFoundConfig)?;
