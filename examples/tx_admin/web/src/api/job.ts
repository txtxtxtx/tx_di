...
export function getJob(id: string) {
  return request.get<ApiRes<JobResponse>>(`/api/v1/job/${id}`).then(r => r.data)
}
...
export function updateJob(id: string, data: Omit<UpdateJobRequest, 'id'>) {
  return request.put<ApiRes<JobResponse>>(`/api/v1/job/${id}`, { ...data, id }).then(r => r.data)
}

/** 删除任务 */
export function deleteJob(id: string) {
  return request.delete<ApiRes<null>>(`/api/v1/job/${id}`).then(r => r.data)
}

/** 修改任务状态 */
export function changeJobStatus(id: string, status: number) {
  return request.put<ApiRes<JobResponse>>(`/api/v1/job/${id}/status`, { id, status }).then(r => r.data)
}

/** 手动执行任务 */
export function runJob(id: string) {
  return request.post<ApiRes<null>>(`/api/v1/job/${id}/run`).then(r => r.data)
}
...
export function getJobLog(id: string) {
  return request.get<ApiRes<JobLogResponse>>(`/api/v1/job/log/${id}`).then(r => r.data)
}
