...
pub struct ConfigModel {
    #[key] #[auto] pub id: u64, #[default("".to_string())] pub category: String, pub config_type: String,
...
impl From<ConfigModel> for Config { fn from(m: ConfigModel) -> Self { Self { id: m.id, category: if m.category.is_empty() { None } else { Some(m.category) }, config_type: if m.config_type=="custom"{ConfigType::Custom}else{ConfigType::BuiltIn}, name: m.name, config_key: m.config_key, value: if m.value.is_empty() { None } else { Some(m.value) }, visible: m.visible, remark: if m.remark.is_empty() { None } else { Some(m.remark) }, creator: if m.creator.is_empty() { None } else { Some(m.creator) }, updater: if m.updater.is_empty() { None } else { Some(m.updater) }, created_at: m.created_at, updated_at: m.updated_at, deleted: m.deleted } } }
...
impl ConfigRepository for ToastyConfigRepository {
    ...
    async fn save(&self, config: &Config) -> Result<(), anyhow::Error> { let mut db = self.toasty.db().clone(); if config.id == 0 { toasty::create!(ConfigModel { category: config.category.clone().unwrap_or_default(), config_type: config.config_type.to_string(), name: config.name.clone(), config_key: config.config_key.clone(), value: config.value.clone().unwrap_or_default(), visible: config.visible, remark: config.remark.clone().unwrap_or_default(), creator: config.creator.clone().unwrap_or_default(), updater: config.updater.clone().unwrap_or_default() }).exec(&mut db).await?; } else { let mut m = ConfigModel::get_by_id(&mut db, config.id).await.map_err(|_| anyhow::anyhow!("not found"))?; m.category = config.category.clone().unwrap_or_default(); m.config_type = config.config_type.to_string(); m.name = config.name.clone(); m.config_key = config.config_key.clone(); m.value = config.value.clone().unwrap_or_default(); m.visible = config.visible; m.remark = config.remark.clone().unwrap_or_default(); m.creator = config.creator.clone().unwrap_or_default(); m.updater = config.updater.clone().unwrap_or_default(); m.update().exec(&mut db).await?; } Ok(()) }
