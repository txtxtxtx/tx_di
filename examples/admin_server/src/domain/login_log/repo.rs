...
pub struct LoginLogModel {
    #[key] #[auto] pub id: u64, pub log_type: String, #[default("".to_string())] pub trace_id: String,
    #[default(0i64)] #[index] pub user_id: i64, #[default(0u8)] pub user_type: u8, #[default("".to_string())] pub username: String,
    pub result: String, #[default("".to_string())] pub user_ip: String, #[default("".to_string())] pub user_agent: String,
...
impl From<LoginLogModel> for LoginLog {
    fn from(m: LoginLogModel) -> Self { Self { id: m.id, log_type: match m.log_type.as_str() { "logout" => LoginLogType::Logout, "force_logout" => LoginLogType::ForceLogout, _ => LoginLogType::Login }, trace_id: if m.trace_id.is_empty() { None } else { Some(m.trace_id) }, user_id: if m.user_id == 0 { None } else { Some(m.user_id as u64) }, user_type: m.user_type, username: if m.username.is_empty() { None } else { Some(m.username) }, result: match m.result.as_str() { "bad_credentials" => LoginResult::BadCredentials, "user_disabled" => LoginResult::UserDisabled, "captcha_missing" => LoginResult::CaptchaMissing, "captcha_wrong" => LoginResult::CaptchaWrong, "unknown" => LoginResult::Unknown, _ => LoginResult::Success }, user_ip: if m.user_ip.is_empty() { None } else { Some(m.user_ip) }, user_agent: if m.user_agent.is_empty() { None } else { Some(m.user_agent) }, tenant_id: m.tenant_id as u64, creator: if m.creator.is_empty() { None } else { Some(m.creator) }, updater: if m.updater.is_empty() { None } else { Some(m.updater) }, created_at: m.created_at, updated_at: m.updated_at, deleted: m.deleted } }
}
...
impl LoginLogRepository for ToastyLoginLogRepository {
    async fn save(&self, log: &LoginLog) -> Result<(), anyhow::Error> {
        let mut db = self.toasty.db().clone(); toasty::create!(LoginLogModel { log_type: log.log_type.to_string(), trace_id: log.trace_id.clone().unwrap_or_default(), user_id: log.user_id.map(|v| v as i64).unwrap_or_default(), user_type: log.user_type, username: log.username.clone().unwrap_or_default(), result: log.result.to_string(), user_ip: log.user_ip.clone().unwrap_or_default(), user_agent: log.user_agent.clone().unwrap_or_default(), tenant_id: log.tenant_id as i64, creator: log.creator.clone().unwrap_or_default(), updater: log.updater.clone().unwrap_or_default() }).exec(&mut db).await?; Ok(())
    }
