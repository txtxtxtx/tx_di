...
pub struct PostModel {
    ...
    #[default(0i32)] pub sort: i32, pub status: String, #[default("".to_string())] pub remark: String, #[index] pub tenant_id: i64,
...
impl From<PostModel> for Post {
    fn from(m: PostModel) -> Self { Self { id: m.id, code: m.code, name: m.name, sort: m.sort, status: if m.status == "disable" { CommonStatus::Disable } else { CommonStatus::Enable }, remark: if m.remark.is_empty() { None } else { Some(m.remark) }, tenant_id: m.tenant_id as u64, creator: if m.creator.is_empty() { None } else { Some(m.creator) }, updater: if m.updater.is_empty() { None } else { Some(m.updater) }, created_at: m.created_at, updated_at: m.updated_at, deleted: m.deleted } }
}
...
impl PostRepository for ToastyPostRepository {
    ...
    async fn save(&self, post: &Post) -> Result<(), anyhow::Error> {
        ...
        if post.id == 0 { toasty::create!(PostModel { code: post.code.clone(), name: post.name.clone(), sort: post.sort, status: post.status.to_string(), remark: post.remark.clone().unwrap_or_default(), tenant_id: post.tenant_id as i64, creator: post.creator.clone().unwrap_or_default(), updater: post.updater.clone().unwrap_or_default() }).exec(&mut db).await?; }
        else { let mut m = PostModel::get_by_id(&mut db, post.id).await.map_err(|_| anyhow::anyhow!("not found"))?; m.code = post.code.clone(); m.name = post.name.clone(); m.sort = post.sort; m.status = post.status.to_string(); m.remark = post.remark.clone().unwrap_or_default(); m.tenant_id = post.tenant_id as i64; m.creator = post.creator.clone().unwrap_or_default(); m.updater = post.updater.clone().unwrap_or_default(); m.update().exec(&mut db).await?; } Ok(())
    }
