...
pub struct DictTypeModel {
    ...
    pub status: CommonStatus, #[default("".to_string())] pub remark: String, #[default("".to_string())] pub creator: String, #[default("".to_string())] pub updater: String,
...
pub struct DictDataModel {
    ...
    #[index] pub dict_type: String, pub status: CommonStatus,
...
impl DictRepository for ToastyDictRepository {
    ...
    async fn save_type(&self, dt: &DictType) -> Result<(), anyhow::Error> { let mut db = self.toasty.db().clone(); if dt.id == 0 { toasty::create!(DictTypeModel { name: dt.name.clone(), dict_type: dt.dict_type.clone(), status: dt.status, remark: dt.remark.clone().unwrap_or_default(), creator: dt.creator.clone().unwrap_or_default(), updater: dt.updater.clone().unwrap_or_default() }).exec(&mut db).await?; } else { let mut m = DictTypeModel::get_by_id(&mut db, dt.id).await.map_err(|_| anyhow::anyhow!("not found"))?; m.name = dt.name.clone(); m.dict_type = dt.dict_type.clone(); m.status = dt.status; m.remark = dt.remark.clone().unwrap_or_default(); m.creator = dt.creator.clone().unwrap_or_default(); m.updater = dt.updater.clone().unwrap_or_default(); m.update().exec(&mut db).await?; } Ok(()) }
    ...
    async fn save_data(&self, data: &DictData) -> Result<(), anyhow::Error> { let mut db = self.toasty.db().clone(); if data.id == 0 { toasty::create!(DictDataModel { sort: data.sort, label: data.label.clone(), value: data.value.clone(), dict_type: data.dict_type.clone(), status: data.status, color_type: data.color_type.clone().unwrap_or_default(), css_class: data.css_class.clone().unwrap_or_default(), remark: data.remark.clone().unwrap_or_default(), creator: data.creator.clone().unwrap_or_default(), updater: data.updater.clone().unwrap_or_default() }).exec(&mut db).await?; } else { let mut m = DictDataModel::get_by_id(&mut db, data.id).await.map_err(|_| anyhow::anyhow!("not found"))?; m.sort = data.sort; m.label = data.label.clone(); m.value = data.value.clone(); m.dict_type = data.dict_type.clone(); m.status = data.status; m.color_type = data.color_type.clone().unwrap_or_default(); m.css_class = data.css_class.clone().unwrap_or_default(); m.remark = data.remark.clone().unwrap_or_default(); m.creator = data.creator.clone().unwrap_or_default(); m.updater = data.updater.clone().unwrap_or_default(); m.update().exec(&mut db).await?; } Ok(()) }
