...
pub struct TenantModel {
    ...
    pub status: TenantStatus, #[default(Vec::new())] #[serialize(json)] pub websites: Vec<String>, #[default(0i64)] pub package_id: i64,
...
pub struct TenantPackageModel {
    #[key] #[auto] pub id: u64, pub name: String, pub status: TenantStatus, #[default("".to_string())] pub remark: String,
...
impl From<TenantModel> for Tenant {
    fn from(m: TenantModel) -> Self { Self { id: m.id, name: m.name, contact_user_id: if m.contact_user_id == 0 { None } else { Some(m.contact_user_id as u64) }, contact_name: if m.contact_name.is_empty() { None } else { Some(m.contact_name) }, contact_mobile: if m.contact_mobile.is_empty() { None } else { Some(m.contact_mobile) }, status: m.status, websites: m.websites, package_id: if m.package_id == 0 { None } else { Some(m.package_id as u64) }, expire_time: None, account_count: m.account_count, creator: if m.creator.is_empty() { None } else { Some(m.creator) }, updater: if m.updater.is_empty() { None } else { Some(m.updater) }, created_at: m.created_at, updated_at: m.updated_at, deleted: m.deleted } }
}
impl From<TenantPackageModel> for TenantPackage {
    fn from(m: TenantPackageModel) -> Self { Self { id: m.id, name: m.name, status: m.status, remark: if m.remark.is_empty() { None } else { Some(m.remark) }, menu_ids: m.menu_ids.iter().filter_map(|s| s.parse().ok()).collect(), creator: if m.creator.is_empty() { None } else { Some(m.creator) }, updater: if m.updater.is_empty() { None } else { Some(m.updater) }, created_at: m.created_at, updated_at: m.updated_at, deleted: m.deleted } }
}
...
impl TenantRepository for ToastyTenantRepository {
    ...
    async fn save(&self, tenant: &Tenant) -> Result<(), anyhow::Error> {
        let mut db = self.toasty.db().clone();
        if tenant.id == 0 { toasty::create!(TenantModel { name: tenant.name.clone(), contact_user_id: tenant.contact_user_id.map(|v| v as i64).unwrap_or_default(), contact_name: tenant.contact_name.clone().unwrap_or_default(), contact_mobile: tenant.contact_mobile.clone().unwrap_or_default(), status: tenant.status, websites: tenant.websites.clone(), package_id: tenant.package_id.map(|v| v as i64).unwrap_or_default(), account_count: tenant.account_count, creator: tenant.creator.clone().unwrap_or_default(), updater: tenant.updater.clone().unwrap_or_default() }).exec(&mut db).await?; }
        else { let mut m = TenantModel::get_by_id(&mut db, tenant.id).await.map_err(|_| anyhow::anyhow!("not found"))?; m.name = tenant.name.clone(); m.contact_user_id = tenant.contact_user_id.map(|v| v as i64).unwrap_or_default(); m.contact_name = tenant.contact_name.clone().unwrap_or_default(); m.contact_mobile = tenant.contact_mobile.clone().unwrap_or_default(); m.status = tenant.status; m.websites = tenant.websites.clone(); m.package_id = tenant.package_id.map(|v| v as i64).unwrap_or_default(); m.account_count = tenant.account_count; m.creator = tenant.creator.clone().unwrap_or_default(); m.updater = tenant.updater.clone().unwrap_or_default(); m.update().exec(&mut db).await?; } Ok(())
    }
