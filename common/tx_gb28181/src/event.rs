//! GB28181 事件类型与全局广播
//!
//! 定义 GB28181 协议中所有可能产生的事件（`Gb28181Event`），以及基于
//! `tokio::sync::broadcast` 的全局事件广播基础设施。
//!
//! ## 设计原则
//!
//! - **事件类型纯定义**：`Gb28181Event` 枚举定义在公共库中，服务端 / 客户端 / 示例程序
//!   均可引用，避免循环依赖。
//! - **广播基础设施**：提供 `subscribe()` / `emit()` / `add_event_listener()` 全局函数，
//!   内部使用 `OnceLock` + `broadcast::channel`，无需框架级初始化即可使用。
...
///
/// 可通过环境变量 `GB28181_EVENT_CHANNEL_CAPACITY` 配置，未设置时使用默认值 4096
fn event_channel_capacity() -> usize {
    std::env::var("GB28181_EVENT_CHANNEL_CAPACITY")
        .ok()
