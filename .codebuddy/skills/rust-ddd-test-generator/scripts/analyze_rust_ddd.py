...
KNOWN LIMITATIONS:
    - Uses regex, not a full Rust parser (syn/rust-analyzer).
    - Cannot detect: pub(crate), pub(super), methods inside trait impls,
      items generated by derive macros (#[derive(...)]), or re-exports (pub use).
    - False negatives are possible. Treat the report as a starting checklist,
      not a complete coverage audit. Use cargo-tarpaulin for accurate coverage.
"""
...
PUB_CRATE_FN = re.compile(r'^\s*pub\s*\(\s*crate\s*\)(?:\s+async)?\s+fn\s+(\w+)')
...
def analyze_file(filepath: Path) -> dict:
    ...
    try:
        ...
        for line in lines:
            ...
            # Also capture pub(crate) fns as they may be part of bounded context API
            m = PUB_CRATE_FN.match(line)
            if m and m.group(1) not in ('new', 'default'):
                result['pub_fns'].append(f"{m.group(1)} [pub(crate)]")
        ...
        has_test = bool(
            TEST_ATTR.search(content) or
            CFG_TEST.search(content) or
            RSTEST.search(content)
        )
        result['has_tests'] = has_test
    except Exception:
        pass
    return result
...
def main():
    ...
    print(f"    (regex-based scan — see KNOWN LIMITATIONS in script header)")
    ...
    if checklist:
        ...
        for layer, path, a in checklist:
            ...
            print(f"  [{layer}] {path}")
            for item in items[:5]:
                # strip [pub(crate)] annotation for test name suggestion
                name = item.split(' ')[0].lower()
                print(f"    → test_{name}_*")
            if len(items) > 5:
    ...
    print(f"\n  Run accurate coverage: cargo tarpaulin --out Html --output-dir coverage/")
    print(f"\n✅  Analysis complete. {len(checklist)} file(s) likely need tests.\n")
