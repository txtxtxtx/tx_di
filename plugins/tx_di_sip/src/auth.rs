//! SIP 摘要认证（RFC 2617 / GB28181）
//!
//! 从 `tx_gb28181::utils` 下沉到 L0 纯净层，供上层插件（平台 / 设备 / 级联）
//! 统一复用，避免各角色重复实现 MD5 与摘要认证逻辑。
//!
//! 本模块**不引入任何 GB 语义**，仅提供通用 SIP 摘要认证原语。
...
/// 构造客户端 `Authorization` 头值（Digest 方案）
///
/// 供需要**主动**向服务器发起认证请求的上层使用（如级联下级注册、设备端注册）。
/// 平台侧入站校验请使用 [`verify_digest_auth`]。
///
/// 返回形如：
/// `Digest username="u", realm="r", nonce="n", uri="sip:...", response="...", algorithm=MD5`
...
/// 从 `WWW-Authenticate` / `Proxy-Authenticate` 头中提取 `nonce` 值
///
/// 输入示例：`Digest realm="3402000000", nonce="def456", algorithm=MD5`
/// 返回：`def456`（未找到时返回空串）
pub fn extract_nonce(auth_str: &str) -> String {
    auth_str
        .split("nonce=\"")
        .nth(1)
...
mod tests {
    ...
    fn md5_known_vector() {
        ...
        assert_eq!(md5_hex(md5_digest(b"")), "d41d8cd98f00b204e9800998ecf8427e");
    }
    ...
    fn digest_auth_valid() {
        ...
        let ha1 = md5_hex(md5_digest(a1.as_bytes()));
        let ha2 = md5_hex(md5_digest(a2.as_bytes()));
        let correct = md5_hex(md5_digest(format!("{}:testnonce:{}", ha1, ha2).as_bytes()));

        let auth = format!(
            "Digest username=\"34020000001320000001\", realm=\"3402000000\", nonce=\"testnonce\", response=\"{}\"",
            correct
        );
    ...
    #[test]
    fn build_digest_authorization_format() {
