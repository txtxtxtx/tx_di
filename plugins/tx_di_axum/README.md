...
**关键导出**：`Router`（`axum::Router` 类型别名）、`DiComp<T>`（handler 提取器）、`WebErr`、`WebErrCode`、`add_layer`/`BodySizeLimitLayer`。
...
5. `WebPlugin.router` 为 `OnceLock`，仅能 set 一次；测试可用 `clear_routers()`/`clear_layers()`。
