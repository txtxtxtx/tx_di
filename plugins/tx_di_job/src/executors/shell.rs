use std::path::Path;
use std::time::Duration;
use async_trait::async_trait;
use tokio::process::Command;
use tokio::time::timeout;
use tracing::{info, warn, error};
use crate::err::JobResult;
use crate::models::ExecutionStatus;
use crate::executors::JobExecutor;

/// Shell 脚本执行器
///
/// 用于执行 Shell 脚本
///
/// # 使用示例
///
/// ```rust,ignore
/// let executor = ShellJobExecutor::new(Duration::from_secs(300));
/// let result = executor.execute(1, "/opt/scripts/backup.sh", Some("--compress")).await;
/// ```
pub struct ShellJobExecutor {
    /// 执行超时时间
    timeout: Duration,
}

impl ShellJobExecutor {
    /// 创建新的 Shell 脚本执行器
    pub fn new(timeout: Duration) -> Self {
        Self { timeout }
    }
    
    /// 设置超时时间
    pub fn with_timeout(mut self, timeout: Duration) -> Self {
        self.timeout = timeout;
        self
    }
}

#[async_trait]
impl JobExecutor for ShellJobExecutor {
    async fn execute(&self, job_id: i64, handler_name: &str, param: Option<&str>) -> JobResult {
        info!(job_id = job_id, script = handler_name, "执行 Shell 脚本");
        
        // 检查脚本是否存在
        if !Path::new(handler_name).exists() {
            error!(job_id = job_id, script = handler_name, "脚本文件不存在");
            return JobResult {
                status: ExecutionStatus::Failed,
                result: None,
                error: Some(format!("脚本文件不存在: {}", handler_name)),
            };
        }
        
        // 构建命令
        let mut cmd = Command::new("bash");
        cmd.arg(handler_name);
        if let Some(p) = param {
            cmd.arg(p);
        }
        
        // 设置超时并执行
        let output = match timeout(self.timeout, cmd.output()).await {
            Ok(result) => {
                match result {
                    Ok(output) => output,
                    Err(e) => {
                        error!(job_id = job_id, error = %e, "执行脚本失败");
                        return JobResult {
                            status: ExecutionStatus::Failed,
                            result: None,
                            error: Some(format!("执行脚本失败: {}", e)),
                        };
                    }
                }
            }
            Err(_) => {
                warn!(job_id = job_id, "脚本执行超时");
                return JobResult {
                    status: ExecutionStatus::Timeout,
                    result: None,
                    error: Some("执行超时".to_string()),
                };
            }
        };
        
        // 检查执行结果
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout).to_string();
            info!(job_id = job_id, "Shell 脚本执行成功");
            JobResult {
                status: ExecutionStatus::Success,
                result: Some(stdout),
                error: None,
            }
        } else {
            let stderr = String::from_utf8_lossy(&output.stderr).to_string();
            error!(job_id = job_id, error = %stderr, "Shell 脚本执行失败");
            JobResult {
                status: ExecutionStatus::Failed,
                result: None,
                error: Some(stderr),
            }
        }
    }
}
