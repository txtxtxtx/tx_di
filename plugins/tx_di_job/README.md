...
1. **Cron 使用 7 字段标准格式（秒 分 时 日 月 周 年）**，与 [`cron`](https://crates.io/crates/cron) crate 完全一致。
   支持 6 字段（省略年）和 5 字段（`min hour dom mon dow`，向后兼容旧数据）。

   | 示例 | 字段数 | 说明 |
   |------|--------|------|
   | `0 30 9,12,15 1,15 May-Aug Mon,Wed,Fri 2018/2` | 7 | 标准格式 |
   | `0 0 2 * * *` | 6 | 每天凌晨 2 点 |
